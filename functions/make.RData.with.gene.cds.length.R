get.cds.length = function(TxDb)
{
  ...
  tmp <- do.call("rbind",lapply(cds.loc.all, function(x) data.frame(cds_id = x@elementMetadata$cds_id[which.max(x@ranges@width)], width = sum(x@ranges@width),stringsAsFactors = F)))
